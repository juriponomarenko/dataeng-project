...
df = pd.read_json('/opt/airflow/dags/project/data/kym.json')
...
df.loc[ind_par_notna,'parent']=df.loc[ind_par_notna,'parent'].apply(lambda x: x.split('memes/')[1].replace('-',' ').lower())
ind_sib_notna=df['siblings'].notna()
df.loc[ind_sib_notna,'siblings']=df.loc[ind_sib_notna,'siblings'].apply(lambda arr: [x.split('memes/')[1].replace('-',' ').lower() for x in arr])
ind_chi_notna=df['children'].notna()
df.loc[ind_chi_notna,'children']=df.loc[ind_chi_notna,'children'].apply(lambda arr: [x.split('memes/')[1].replace('-',' ').lower() for x in arr])
...
df.loc[ind_chi_isna,'children'] = df.loc[ind_chi_isna,'siblings'].apply(lambda x:[])
...
df_final.to_json("/opt/airflow/dags/project/data/kym_cleaned.json",orient='records')